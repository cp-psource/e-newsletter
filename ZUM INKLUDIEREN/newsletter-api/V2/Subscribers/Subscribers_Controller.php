...
class Subscribers_Controller extends REST_Controller {
    ...
    public function register_routes() {
        ...
        // With ID
        register_rest_route($this->namespace, "/$this->rest_base/(?P<_id>[\d]+)",
                array(
                    //'schema' => array($this, 'get_public_item_schema'),
                    'args' => array(
                        '_id' => array(
                    ...
                    array(
                        ...
                        'callback' => array($this, 'update_item_from_id'),
                        'permission_callback' => array($this, 'permissions_check')
                    ),
        ...
        // With email
        register_rest_route($this->namespace, "/$this->rest_base/(?P<_email>[\S-]+)",
                array(
                    //'schema' => array($this, 'get_public_item_schema'),
                    'args' => array(
                        '_email' => array(
    ...
    public function get_item($request) {
        $subscriber = \Newsletter::instance()->get_user($request->get_param('_id'));

        if (!$subscriber) {
	        \NewsletterApi::$instance->get_logger()->debug('404 Subscriber does not exist');
    ...
    public function get_item_by_email($request) {
        $subscriber_email = $request->get_param('_email');
    ...
    public function update_item_from_id($request) {
    ...
    public function update_or_create_item_from_email($request) {
        ...
        $subscriber = Newsletter::instance()->get_user($request->get_param('_email'));

        // When the email does not match a subscriber, we need add the email address to the
        // params if not present, otherwise the creation procedure does not find it.
        if (!$subscriber) {
            $request->set_param('email', $request->get_param('_email'));
        }
        ...
        if (is_null($subscriber)) {
            ...
            if ($prepared_subscriber->email != $subscriber->email) {
                ...
                if ($can_subscriber_email_be_updated instanceof WP_Error) {
	                \NewsletterApi::$instance->get_logger()->debug('400 The email is already present or cannot be changed');
                    return new WP_REST_Response(__('The email is already present or cannot be changed', 'newsletter'), 400);
                }
            }
            ...
            if (false === $subscriber || is_null($subscriber)) {
	            \NewsletterApi::$instance->get_logger()->debug('500 Error on subscriber data update');
    ...
    private function can_subscriber_email_be_updated($new_email, $old_subscriber_id) {
        ...
        if (!is_null($user_of_new_email) && $user_of_new_email->id != $old_subscriber_id) {
	        \NewsletterApi::$instance->get_logger()->debug('The email is already present');
    ...
    private function delete_subscriber($subscriber) {
        ...
        if (is_null($subscriber)) {
	        \NewsletterApi::$instance->get_logger()->debug('400 Subscriber does not exist');
    ...
    protected function prepare_item_for_database($request) {
	    ...
	    \NewsletterApi::$instance->get_logger()->debug($subscriber);
    ...
    public function prepare_item_for_response($subscriber, $request) {
        ...
        $subscriber_dto->token = $subscriber->token;

	    \NewsletterApi::$instance->get_logger()->debug($subscriber_dto);
