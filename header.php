...
<div id="tnp-menu">
    <ul>
        <li>
            <a href="?page=newsletter_main_index"><img src="<?php echo plugins_url('e-newsletter'); ?>/admin/images/logo.png" class="tnp-header-logo" style="vertical-align: bottom;"></a>
        ...
        if (Newsletter::instance()->is_multilanguage()) {
            ...
            if ($tnp_page_status === false) {
                ...
                if (strpos($content, '[newsletter]') === false && strpos($content, '[newsletter ') === false) {
                    ...
                    ?>
                    <div class="tnp-notice tnp-notice-warning">
                        The ClassicPress page "<?php echo esc_html($title) ?>" set as Newsletter's public page on the <a href="admin.php?page=newsletter_main_main">main settings</a>
...
if (isset($_GET['debug']) || !defined('NEWSLETTER_CRON_WARNINGS') || NEWSLETTER_CRON_WARNINGS) {
    ...
    if (isset($_GET['debug']) || $x !== NewsletterSystemAdmin::JOB_OK) {
        echo '<div class="tnp-notice tnp-notice-warning">The ClassicPress scheduler is not working properly: that affects the newsletter sending and other site tasks.<br>';
