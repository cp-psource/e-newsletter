<?php
namespace Psource\PluginUpdateChecker\v5p0\Plugin;
	...
	class Ui {
		...
		public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
			if ( $this->isMyPluginFile($pluginFile) ) {
				...
				$linkText = apply_filters(
					...
					__('Auf neuere Version prüfen', 'email-newsletter')
				);
				if ( !empty($linkText) ) {
		...
		public function displayManualCheckResult() {
			...
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->updateChecker->slug) ) {
				...
				if ( $status == 'no_update' ) {
					$message = sprintf(_x('Keine neuere Version von %s im PSOURCE REPO gefunden.', 'the plugin title', 'email-newsletter'), $title);
				} else if ( $status == 'update_available' ) {
					$message = sprintf(_x('Im PSOURCE REPO ist eine <b>neuere Version von %s</b> verfügbar!.', 'the plugin title', 'email-newsletter'), $title);
				} else if ( $status === 'error' ) {
					$message = sprintf(_x('Entschuldige, es konnte nicht festgestellt werden, ob Updates für %s verfügbar sind.', 'the plugin title', 'email-newsletter'), $title);
