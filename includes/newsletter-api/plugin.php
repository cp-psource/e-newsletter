...
//        }
    ...
    function get_users($args, $format = OBJECT) {
        ...
        if (empty($args['per_page'])) $args['per_page'] = 10;
    ...
    function hook_rest_api_init($server) {
        ...
        if (isset($GLOBALS['wp']->query_vars['rest_route'])) {
            ...
            if (strpos($route, '/newsletter/v2') === 0) {
                ...
                $logger->info($_SERVER['REQUEST_METHOD'] . ' ' . $route);
                $request_input = file_get_contents('php://input');
                $logger->debug($request_input);
                Newsletter\Logs::add('api', $_SERVER['REQUEST_METHOD'] . ' ' . $route, '', mb_substr($request_input, 0, 1024));
            }
        }
